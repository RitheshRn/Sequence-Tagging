...
#             optimal_tag_sequence.append(best_v)
    ...
    def read_dev(self, magic):
        ...
        for idx, line in enumerate(lines):
            if len(line) > 0:
                try:
                    ...
                    elif len(line) > float(magic):
                        temp = sorted(self.emission['_LEN_'], key=lambda x: x[1], reverse=True)[0][0]
                    else:
        ...
        with open('gene_dev.LEN.out','w') as f:
            for i in lines:
                f.write(i)
